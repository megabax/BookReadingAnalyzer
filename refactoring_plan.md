...
| Загрузка Selenium | фоновая / устойчивая к смене вкладок (не должне проадать индикатор загрузки) | ⏳ этап C |

**Этап C — требования к загрузке:**
- желательно не блокировать построение отчётов на других вкладках (фон / job);
- **при переключении вкладок Streamlit загрузка не должна исчезать** (спиннер/статус/сессия Selenium и пауза на код устройства сохраняются; возврат на «Загрузку» показывает тот же прогресс).
