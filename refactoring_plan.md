...
| Загрузка Selenium | фоновый FetchJob + прогресс + баннер | ✅ этап C |

**Этап C — требования к загрузке:**
- ✅ не блокировать построение отчётов на других вкладках (`FetchJob` в фоне);
- ✅ **при переключении вкладок загрузка не исчезает** (баннер над вкладками + статус на «Загрузке»);
- ✅ пауза на код устройства / 2FA с «Продолжить»;
- ✅ прогресс: стадия, порция `i/N`, `st.progress`.
