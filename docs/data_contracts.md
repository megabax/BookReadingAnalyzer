...
**Актуальные данные для отчётов — MS SQL** (`fetch_runs`, `chapter_reads`).  
JSON в `data/raw/` — **устаревший** промежуточный формат (промежуточная версия пайплайна). Код чтения JSON сохранён для тестов и отладки (`AT_ENABLE_LEGACY_JSON=yes`); из UI и CLI по умолчанию не используется. Удаление JSON-пути — отдельная задача после полного перехода на репозиторий.

## JSON снимок (`data/raw/reads_*.json`) — устаревший
