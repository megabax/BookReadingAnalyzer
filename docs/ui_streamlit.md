...
4. **Долгие операции** (Selenium) — не в синхронной кнопке; отдельный этап (фон, subprocess с логом, или CLI). **Обязательно:** переключение вкладок UI не должно сбрасывать/прятать идущую загрузку (статус и сессия сохраняются).
...
- Кнопка «Загрузить период» → фоновый `FetchJob` (поток): на «Воронке»/«Сравнении» можно строить отчёты параллельно
- Поле кода нового устройства; пауза «Продолжить» без закрытия браузерной сессии
- Статус: running / awaiting_code / done / error / cancelled; данные в MS SQL
- **Переключение вкладок не отменяет загрузку:** баннер над вкладками + прогресс (стадия, порция `i/N`, progress bar)
- Headless по умолчанию (`AT_HEADLESS`)

### Не в MVP

- Мультипользовательский доступ, регистрация
- Редактирование данных в БД (кроме будущей кнопки «удалить run» с подтверждением)
- Продажи из `reclan.csv` (вне текущего scope)
...
| Зависание UI на Selenium | Этап C: фон / job |
| Сброс загрузки при смене вкладки | Сохранять статус и сессию вне отрисовки вкладки; не терять spinner/awaiting_code |
