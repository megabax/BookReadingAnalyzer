...
- Кнопка «Загрузить период» → фоновый процесс / отдельное окно терминала (чтобы на «Воронке»/«Сравнении» можно было строить отчёты параллельно)
- Поле кода нового устройства (как в CLI); пауза «Продолжить» без закрытия браузера
- Статус: running / done / error; данные в MS SQL
- **Переключение вкладок не отменяет загрузку:** индикатор/статус и Selenium-сессия не пропадают; при возврате на «Загрузку» виден тот же прогресс (или ожидание кода)
