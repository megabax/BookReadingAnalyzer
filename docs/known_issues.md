...
**Ускорение (2026-08):** сначала только `dataSource`; DOM-scroll — fallback, если dataSource пуст/недоступен (раньше гонялись оба пути).
...
Повторная загрузка того же периода **добавляет** новый run; аналитика **суммирует** `metric_value`. Очистка: `scripts/delete_runs.py` по `work_id` + диапазон `fetched_at`.
